...
export default function WaterScreen() {
  ...
  const weeklyAvg =
    weeklyWater.length > 0
      ? weeklyWater.reduce((s, d) => s + d.amount, 0) / weeklyWater.length
      : 0;
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <Card style={[styles.mainCard, { borderColor: `${colors.blue}30` }]} elevated>
          <LinearGradient
            colors={isDark ? ['#0C1A3A', '#0A0A0F'] : ['#EFF6FF', '#F8FAFC']}
          ...
          <View style={styles.bottleContainer}>
            <View style={[styles.bottle, { borderColor: `${colors.blue}40`, backgroundColor: `${colors.blue}10` }]}>
              <View style={[styles.bottleFill, { height: `${pct}%` }]}>
                <LinearGradient
                  colors={pct >= 75 ? [colors.blue, '#60A5FA'] : pct >= 50 ? [colors.blue, '#93C5FD'] : ['#1D4ED8', colors.blue]}
                  style={StyleSheet.absoluteFill}
                  start={{ x: 0, y: 1 }}
                  end={{ x: 1, y: 0 }}
                />
          ...
          <View style={styles.quickGrid}>
            {QUICK_AMOUNTS.map(({ label, amount, emoji, desc }) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <LinearGradient colors={[`${colors.blue}20`, `${colors.blue}05`]} style={StyleSheet.absoluteFill} />
              ...
              return (
                <View key={m.label} style={styles.milestone}>
                  <View style={[styles.milestoneCircle, { borderColor: colors.border }, reached && { backgroundColor: colors.blue, borderColor: colors.blue }]}>
                    <Text style={[styles.milestoneIcon, { color: reached ? 'white' : colors.textMuted }]}>
                      {reached ? '✓' : m.label}
                    </Text>
                  </View>
              ...
              return (
                <View key={d.day} style={styles.weeklyBarCol}>
                  ...
                  <View style={[styles.weeklyBarTrack, { backgroundColor: colors.surface }]}>
                    <LinearGradient
                      colors={isToday ? [colors.primary, colors.primaryLight] : [colors.blue, '#60A5FA']}
                      style={[styles.weeklyBarFill, { height: `${barPct * 100}%` }]}
                      start={{ x: 0, y: 1 }}
                      end={{ x: 0, y: 0 }}
                    />
                  </View>
                  <Text style={[styles.weeklyBarDay, { color: colors.textMuted }, isToday && { color: colors.primary, fontWeight: '800' }]}>
                    {d.day}
                  </Text>
                </View>
              );
...
const styles = StyleSheet.create({
  ...
  mainCard: { overflow: 'hidden', gap: SPACING.lg, padding: SPACING.xl },
  bottleContainer: { flexDirection: 'row', alignItems: 'center', gap: SPACING.xl, justifyContent: 'center' },
  bottle: { width: 110, height: 180, borderRadius: 20, borderWidth: 2, overflow: 'hidden', justifyContent: 'flex-end', position: 'relative' },
  ...
  bottleStatVal:     { fontSize: FONT.xxl, fontWeight: '900' },
  bottleStatUnit:    { fontSize: FONT.xs, fontWeight: '600' },
  bottleStatDivider: { height: 1, width: 40 },
  message:  { textAlign: 'center', fontSize: FONT.md, fontWeight: '800' },
  dropsRow: { flexDirection: 'row', justifyContent: 'center', gap: SPACING.sm },
  drop:     { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  dropEmoji: { fontSize: 14 },
  quickSection: { gap: SPACING.md },
  sectionTitle: { fontSize: FONT.md, fontWeight: '800' },
  quickGrid: { flexDirection: 'row', gap: SPACING.sm },
  quickBtn:   { flex: 1, borderRadius: RADIUS.md, borderWidth: 1, padding: SPACING.md, alignItems: 'center', gap: 4, overflow: 'hidden' },
  quickEmoji: { fontSize: 24 },
  quickLabel: { fontSize: FONT.sm, fontWeight: '900' },
  quickDesc:  { fontSize: FONT.xs, fontWeight: '700' },
  quickMl:    { fontSize: FONT.xs },
  ...
  milestoneCircle: { width: 44, height: 44, borderRadius: 22, borderWidth: 2, alignItems: 'center', justifyContent: 'center' },
  milestoneIcon:   { fontSize: FONT.sm, fontWeight: '800' },
  ...
  weeklyAvg:    { fontSize: FONT.sm, fontWeight: '700' },
  ...
  weeklyBarAmt:  { fontSize: FONT.xs, fontWeight: '600' },
  weeklyBarTrack: { flex: 1, width: '100%', borderRadius: RADIUS.sm, overflow: 'hidden', justifyContent: 'flex-end' },
  weeklyBarFill:  { width: '100%', borderRadius: RADIUS.sm },
  weeklyBarDay:   { fontSize: FONT.xs, fontWeight: '700' },
  goalLineLabel:  { fontSize: FONT.xs, textAlign: 'center', marginTop: SPACING.xs },
  logCard:   { gap: SPACING.sm },
  logRow:    { flexDirection: 'row', alignItems: 'center', gap: SPACING.md, paddingVertical: SPACING.sm, borderTopWidth: 1 },
  logEmoji:  { fontSize: 18 },
  logAmount: { fontSize: FONT.md, fontWeight: '800', flex: 1 },
  logTime:   { fontSize: FONT.sm },
