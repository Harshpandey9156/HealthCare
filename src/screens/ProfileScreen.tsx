...
export default function ProfileScreen() {
  ...
  const { user, leaderboard, workoutLogs, logout, fetchProfile } = useAppStore();
  const [activeTab, setActiveTab] = useState<'stats' | 'streaks' | 'leaderboard' | 'settings'>('stats');
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {/* Hero */}
        <LinearGradient colors={isDark ? ['#0F0F1F', '#1A0A2E'] : ['#EEF2FF', '#F5F3FF']} style={styles.hero}>
          <View style={styles.avatarContainer}>
            <LinearGradient colors={['#6366F1', '#EC4899']} style={styles.avatar}>
              <Text style={styles.avatarText}>{user?.name?.charAt(0) || 'U'}</Text>
            </LinearGradient>
            {user?.isPremium && (
              <View style={[styles.crownBadge, { backgroundColor: colors.bg }]}>
              ...
              <>
                <View style={styles.levelProgressRow}>
                  ...
                  <Text style={[styles.levelProgressLabel, { color: nextLevel.color || colors.yellow }]}>
                    {nextLevel.minPts - pts} pts → {nextLevel.name}
                  </Text>
                </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabsScroll} contentContainerStyle={styles.tabsContent}>
          {TABS.map((t) => (
            <TouchableOpacity
              key={t.key}
              style={[styles.tab, { borderColor: activeTab === t.key ? colors.primary : colors.border }, activeTab === t.key && { backgroundColor: colors.primary }]}
        ...
        {activeTab === 'stats' && (
          <View style={styles.tabContent}>
            ...
            <View style={[styles.referCard, { borderColor: `${colors.primary}40` }]}>
              <LinearGradient colors={[isDark ? '#1E1B4B' : '#EEF2FF', colors.card]} style={StyleSheet.absoluteFill} />
            ...
            <Card style={[styles.section, { backgroundColor: colors.card }]}>
              ...
              {leaderboard.map((entry, i) => (
                <View key={i} style={[styles.leaderRow, entry.isCurrentUser && { borderColor: `${colors.primary}50`, borderWidth: 1 }, { overflow: 'hidden', borderRadius: RADIUS.md }]}>
                  {entry.isCurrentUser && (
                    <LinearGradient colors={[isDark ? '#1E1B4B' : '#EEF2FF', 'transparent']} style={StyleSheet.absoluteFill} />
                  )}
                  ...
                  <View style={styles.leaderInfo}>
                    <Text style={[styles.leaderName, { color: entry.isCurrentUser ? colors.primary : colors.textPrimary }]}>
              ...
              <View style={styles.themeRow}>
                {([
                  ...
                  return (
                    <TouchableOpacity
                      key={opt.mode}
                      style={[styles.themeOption, { borderColor: active ? colors.primary : colors.border, backgroundColor: active ? `${colors.primary}15` : colors.surface }]}
